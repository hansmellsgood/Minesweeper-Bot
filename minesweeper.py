...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        cell_neighbors = cell_neighbors - self.safes - self.moves_made
        ...
        for sentence in self.knowledge:
            if len(sentence.cells) == 0:
                ...
                for cell in sentence.known_mines():
                    updated_mines.add(cell)
            elif sentence.known_safes():
                for cell in sentence.known_safes():
                    updated_safes.add(cell)
        if updated_mines:
            for cell in updated_mines:
                self.mark_mine(cell)
        if updated_safes:
            for cell in updated_safes:
                self.mark_safe(cell)
        ...
        for sentence1 in self.knowledge:
            for sentence2 in self.knowledge:
                ...
                if sentence1.cells.issubset(sentence2.cells):
                    # new sentence's cell
                    n_cells = sentence2.cells - sentence1.cells
                    # new sentence's count
                    n_count = sentence2.count - sentence1.count
                    # new sentence
                    n_sentence = Sentence(n_cells, n_count)

                    known_mines = n_sentence.known_mines()
                    known_safes = n_sentence.known_safes()
                    if known_mines:
    ...
    def make_random_move(self):
        ...
        for i in range(0, self.height):
            for j in range(0, self.width):
                total_moves.add((i, j))
        remaining_moves = total_moves - self.mines - self.moves_made
        if remaining_moves:
